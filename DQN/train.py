...
if __name__ == '__main__':
    ...
    rewards_log = train(env,agent,NUM_EPISODE,EPS_INIT,EPS_DENCY,EPS_MIN,MAX_T,NUM_FRAME,CONSTANT)
    env.close()
    np.save('{}_rewards.npy'.format(ENV_NAME),rewards_log)
    agent.Q_local.to('cpu')
    torch.save(agent.Q_local.state_dict(),'{}weights.pth'.format(ENV_NAME))